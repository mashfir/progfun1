...
object Anagrams extends AnagramsInterface {
  ...
  def wordOccurrences(w: Word): Occurrences = {
    w.toLowerCase.toList.groupBy(c => c).toList.map({case (char, list) => (char, list.length)}).sorted
  }
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] =
    (occurrences foldRight List[Occurrences](Nil)) {case ((c, occ), acc) =>
      acc ++ (for (comb <- acc; times <- 1 to occ) yield (c, times) :: comb)
    }
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = {
    (y foldLeft SortedMap[Char, Int]() ++ x ) {case (map, (c, occ)) => {
      val newOcc = map(c) - occ
      if (newOcc != 0) map updated (c, newOcc)
      else map - c
    }}.toList
  }
